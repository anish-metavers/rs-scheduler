...
@Injectable()
export class Sportid1CronService {
  constructor(@Inject(CACHE_MANAGER) private cacheManager: Cache) {}
  private readonly logger = new Logger(Sportid1CronService.name);

  @Cron('*/1 * * * *')
  async handleCron() {
    this.logger.debug('Active match cron started with sportid 1');
    ...
    try {
      ...
      and TM.sportid = 1 and TM.isactive = 1
